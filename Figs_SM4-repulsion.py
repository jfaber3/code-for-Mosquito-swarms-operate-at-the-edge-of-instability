...
def find_repulsion_force(pos_data, acc_data, data_mask):
    print("Finding repulsion force")
    T = pos_data.shape[1]
    Dist, Acc_par = [], []
    for t in range(T):
        mask_t = data_mask[:, t]
        ...
        #calculating unit vector to avg of k nearest neghbors
        k = 1
        sorted_idx = np.argsort(D_mag, axis=1)    #sort distances to get nearest neighbors
        kNN = sorted_idx[:, :k]                   # (N, k)  indices of k nearest neighbors for each point
        NN_vecs = Pos[kNN] - Pos[:, None, :]   # (N, k, 3)  direction vectors to neighbors
        avg_NN_vecs = np.mean(NN_vecs, axis=1)
        avg_NN_unit = avg_NN_vecs/np.linalg.norm(avg_NN_vecs, axis=1)[:, None]
        scalar_projs = np.sum(Acc * avg_NN_unit, axis=1)
        Dist.extend(np.linalg.norm(avg_NN_vecs, axis=1))
        Acc_par.extend(scalar_projs)
    return np.array(Dist), np.array(Acc_par)


Dist1, Acc_par1 = find_repulsion_force(pos_data1, acc_data1, data_mask1)
Dist2, Acc_par2 = find_repulsion_force(pos_data2, acc_data2, data_mask2)
Dist3, Acc_par3 = find_repulsion_force(pos_data3, acc_data3, data_mask3)
Dist4, Acc_par4 = find_repulsion_force(pos_data4, acc_data4, data_mask4)
Dist5, Acc_par5 = find_repulsion_force(pos_data5, acc_data5, data_mask5)
Dist6, Acc_par6 = find_repulsion_force(pos_data6, acc_data6, data_mask6)



def plot_acc_par_range(Dist, Acc_par, dmins, dmaxs, clr, num_hist_bins=51):
    Acc_par_a = Acc_par[ (Dist >= dmins[0]) & (Dist <= dmaxs[0]) ]
    Acc_par_b = Acc_par[ (Dist >= dmins[1]) & (Dist <= dmaxs[1]) ]
    Acc_par_c = Acc_par[ (Dist >= dmins[2]) & (Dist <= dmaxs[2]) ]
    Acc_par_d = Acc_par[ (Dist >= dmins[3]) & (Dist <= dmaxs[3]) ]
    Acc_par_e = Acc_par[ (Dist >= dmins[4]) & (Dist <= dmaxs[4]) ]
    Acc_par_f = Acc_par[ (Dist >= dmins[5]) & (Dist <= dmaxs[5]) ]
    ax1.hist(Acc_par_a, bins=num_hist_bins, range=(-10, 10), histtype='step', color=clr, density=True)
    ax2.hist(Acc_par_b, bins=num_hist_bins, range=(-10, 10), histtype='step', color=clr, density=True)
    ax3.hist(Acc_par_c, bins=num_hist_bins, range=(-10, 10), histtype='step', color=clr, density=True)
    ax4.hist(Acc_par_d, bins=num_hist_bins, range=(-10, 10), histtype='step', color=clr, density=True)
    ax5.hist(Acc_par_e, bins=num_hist_bins, range=(-10, 10), histtype='step', color=clr, density=True)
    ax6.hist(Acc_par_f, bins=num_hist_bins, range=(-10, 10), histtype='step', color=clr, density=True)
    ax1.axvline(x=Acc_par_a.mean(), ls="dotted", color=clr)
    ax2.axvline(x=Acc_par_b.mean(), ls="dotted", color=clr)
    ax3.axvline(x=Acc_par_c.mean(), ls="dotted", color=clr)
    ax4.axvline(x=Acc_par_d.mean(), ls="dotted", color=clr)
    ax5.axvline(x=Acc_par_e.mean(), ls="dotted", color=clr)
    ax6.axvline(x=Acc_par_f.mean(), ls="dotted", color=clr)
    print(len(Acc_par_a), len(Acc_par_b), len(Acc_par_c), len(Acc_par_d), len(Acc_par_e), len(Acc_par_f))
...
plot_acc_par_range(Dist1, Acc_par1, Dmins, Dmaxs, clrs[1], num_hist_bins=Num_Bins)
plot_acc_par_range(Dist2, Acc_par2, Dmins, Dmaxs, clrs[2], num_hist_bins=Num_Bins)
plot_acc_par_range(Dist3, Acc_par3, Dmins, Dmaxs, clrs[3], num_hist_bins=Num_Bins)
plot_acc_par_range(Dist4, Acc_par4, Dmins, Dmaxs, clrs[4], num_hist_bins=Num_Bins)
plot_acc_par_range(Dist5, Acc_par5, Dmins, Dmaxs, clrs[5], num_hist_bins=Num_Bins)
plot_acc_par_range(Dist6, Acc_par6, Dmins, Dmaxs, clrs[6], num_hist_bins=Num_Bins)
...
ax6.set_xlabel("Acceleration ($m/s^2$)")
ax1.set_ylabel("Probability density")
ax3.set_ylabel("Probability density")
ax5.set_ylabel("Probability density")

#plt.savefig("C:/Users/Justin/Desktop/Fig-S6_repulsion.pdf", dpi=300)
